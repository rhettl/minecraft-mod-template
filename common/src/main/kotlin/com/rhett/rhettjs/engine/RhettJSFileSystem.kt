...
class RhettJSFileSystem(
    ...
    companion object {
        private val BUILT_IN_MODULES = setOf("World", "StructureNbt", "LargeStructureNbt", "WorldgenStructure", "Store", "NBT", "Server", "Commands", "Runtime", "Script")
        private const val VIRTUAL_PREFIX = "/__builtins__/"

        // Map submodule paths to their API names
        // Note: "rhettjs/structure" is handled specially to export both StructureNbt and LargeStructureNbt
        private val SUBMODULE_MAP = mapOf(
            ...
            "rhettjs/worldgen-structure" to "WorldgenStructure",
            "rhettjs" to "rhettjs" // Barrel import
        )

        // Special submodules that export multiple APIs
        private val MULTI_API_SUBMODULES = setOf("rhettjs/structure")
    }
    ...
    private fun isBareBuiltInSpecifier(specifier: String): Boolean {
        ...
        if (normalized.startsWith("rhettjs")) {
            val isBuiltIn = normalized in SUBMODULE_MAP || normalized in MULTI_API_SUBMODULES || normalized == "rhettjs"
            if (isBuiltIn) {
    ...
    private fun extractModuleName(specifier: String): String {
        ...
        if (normalized.startsWith("rhettjs")) {
            // Multi-API submodules keep their name for special handling
            if (normalized in MULTI_API_SUBMODULES) {
                return normalized
            }
            // Regular submodules map to a single API
    ...
    private fun generateBuiltInModule(name: String): String {
        ...
        if (name == "rhettjs") {
            val moduleCode = """
                ...
                export { default as WorldgenStructure } from '/__builtins__/WorldgenStructure';
        ...
        // Special handling for rhettjs/structure submodule (exports both StructureNbt and LargeStructureNbt)
        if (name == "rhettjs/structure" || name == "structure") {
            val moduleCode = """
                // RhettJS Structure Module (StructureNbt + LargeStructureNbt)
                // Generated by RhettJSFileSystem
                export { default as StructureNbt } from '/__builtins__/StructureNbt';
                export { default as LargeStructureNbt } from '/__builtins__/LargeStructureNbt';
            """.trimIndent()
            ConfigManager.debug("Generated structure submodule for '$name':\n$moduleCode")
            return moduleCode
        }
